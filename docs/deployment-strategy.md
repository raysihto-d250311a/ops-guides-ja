...
1. [環境構成](#環境構成)
2. [ブランチ・タグと環境の対応](#ブランチ・タグと環境の対応)
3. [タグの種類と命名規則](#タグの種類と命名規則)
4. [RC タグの作成場所](#rc-タグの作成場所)
5. [アプリの配布 (Distribute)](#アプリの配布-distribute)
6. [デプロイフロー図](#デプロイフロー図)
7. [デプロイ・配布と動作確認の省略](#デプロイ・配布と動作確認の省略)
...
3. 審査リジェクトの場合:
   1. `master` ブランチから `hotfix/*` ブランチを分岐
   2. 修正を行い、`hotfix/*` ブランチ上で RC タグ (`vX.Y.Z-rc.M`) を作成
   3. STG で動作確認
   4. `master` にマージして新しい RC タグを作成して再提出
...
📘 **手順の詳細**: [ブランチ・デプロイ運用手順書 - デプロイ・配布と動作確認を省略する](./branching-and-deployment-runbook.md#デプロイ配布と動作確認を省略する)
