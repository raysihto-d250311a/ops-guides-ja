...
| `vX.Y.Z` タグ (master 上) | PRD 環境 (Web のみ) | 原則として手動 / 半自動 |
| `vX.Y.Z-rc.N` タグ (審査パス済み) | PRD 環境 (モバイルのみ) | ストアリリース |
...
- **Web アプリ**: `vX.Y.Z` タグを PRD 環境にデプロイします
- **モバイルアプリ**: 審査にパスした `vX.Y.Z-rc.N` タグのビルドがストアリリースの対象となります（正式タグ `vX.Y.Z` からビルド・配布は行いません）
...
4. 審査パス後、**審査にパスした RC タグと同じコミットに対して** `vX.Y.Z` タグを作成
5. 正式タグ `vX.Y.Z` のビルドは審査に再提出しない（RC タグのビルドと同一であるため）

> **重要**: 
> - 審査に提出・通過するのは RC タグ（`vX.Y.Z-rc.N`）のビルドです
> - 正式タグ `vX.Y.Z` は、審査にパスした RC タグと同一のコミットに対して作成しますが、このタグからビルドを作成して審査に提出することはありません
> - ストアにリリースされるのは、審査にパスした RC タグのビルドです
...
[master] へマージ
    │
    ↓
[vX.Y.Z-rc.N] タグ (master 上) ─────────→ STG 配布 (審査提出) ──→ 審査パス ──→ PRD 配布 (Store リリース)
    │
    ↓ 審査パス後
...
> **ポイント**: 
> - ストア審査に提出し、審査パス後にストアリリースされるのは **RC タグ（`vX.Y.Z-rc.N`）のビルド**です
